...
static inline float surrogate_u_r_now(const RSO* r, const OrbitalEyesSandbox* env) {
    if (env->uncertainty_mode == 2) return r->u_r;
    float age = r->age;
    return r->u_r + (env->growth_a_r + 0.5f * env->growth_b_r * age) * age;
}
static inline float surrogate_u_t_now(const RSO* r, const OrbitalEyesSandbox* env) {
    if (env->uncertainty_mode == 2) return r->u_t;
    float age = r->age;
    return r->u_t + (env->growth_a_t + 0.5f * env->growth_b_t * age) * age;
}
static inline float surrogate_u_n_now(const RSO* r, const OrbitalEyesSandbox* env) {
    if (env->uncertainty_mode == 2) return r->u_n;
    float age = r->age;
    return r->u_n + (env->growth_a_n + 0.5f * env->growth_b_n * age) * age;
}
static inline float surrogate_sigma_vel_now(const RSO* r, const OrbitalEyesSandbox* env) {
...
static inline float surrogate_u_sum_now(const RSO* r, const OrbitalEyesSandbox* env) {
...
static inline void ekf_sync_rso_uncertainty(RSO* r, const OrbitalEyesSandbox* env) {
...
void init(OrbitalEyesSandbox* env) {
...
static float compute_obs_quality(
    OrbitalEyesSandbox* env, const float* sat_pos, const float* sat_dir,
...
static float compute_population_objective(OrbitalEyesSandbox* env) {
    float sum_U = 0.0f;
    float max_U = 0.0f;
...
static float compute_catalogue_badness(
    OrbitalEyesSandbox* env, float* out_tail_badness, float* out_frac_above) {
...
static float compute_tail_sensitive_badness(OrbitalEyesSandbox* env) {
...
static float compute_precision_badness(OrbitalEyesSandbox* env) {
    float sum_U = 0.0f;
    float max_U = 0.0f;
...
static float precision_goal_badness(OrbitalEyesSandbox* env) {
...
static float compute_fine_max_score(OrbitalEyesSandbox* env) {
...
static float compute_fine_mean_score(OrbitalEyesSandbox* env) {
...
// Forward declaration
static void compute_observations(OrbitalEyesSandbox* env);
static void set_was_my_target(OrbitalEyesSandbox* env);

// ─── Reset ───

static void set_ground_sensor_state(OrbitalEyesSandbox* env, Satellite* s, int reset_pointing) {
    float lat = env->ground_lat_deg * DEG2RAD;
    float lon = env->ground_lon_deg * DEG2RAD;
    float radius = EARTH_RADIUS_KM + env->ground_alt_km;
...
void c_reset(OrbitalEyesSandbox* env) {
    env->tick = 0;
    env->epoch_time = 0.0f;
...
static void compute_observations(OrbitalEyesSandbox* env) {
    ...
    for (int a = 0; a < env->num_agents; a++) {
        ...
        for (int j = 0; j < env->num_agents; j++) {
            ...
            obs[base + 0] = tanhf((other->pos[0] - sat->pos[0]) / env->obs_dist_norm);
            obs[base + 1] = tanhf((other->pos[1] - sat->pos[1]) / env->obs_dist_norm);
            obs[base + 2] = tanhf((other->pos[2] - sat->pos[2]) / env->obs_dist_norm);
        ...
        for (int k = 0; k < env->rso_top_k; k++) {
            ...
            // Relative position
            obs[base + 0] = tanhf((rso->pos[0] - sat->pos[0]) / env->obs_dist_norm);
            obs[base + 1] = tanhf((rso->pos[1] - sat->pos[1]) / env->obs_dist_norm);
            obs[base + 2] = tanhf((rso->pos[2] - sat->pos[2]) / env->obs_dist_norm);

            // Relative velocity
            obs[base + 3] = tanhf((rso->vel[0] - sat->vel[0]) / env->obs_vel_norm);
            obs[base + 4] = tanhf((rso->vel[1] - sat->vel[1]) / env->obs_vel_norm);
            obs[base + 5] = tanhf((rso->vel[2] - sat->vel[2]) / env->obs_vel_norm);
...
// ─── Step ───
void c_step(OrbitalEyesSandbox* env) {
    ...
    if (env->reward_clip_max > 0.0f && r_direct > env->reward_clip_max) {
        r_direct = env->reward_clip_max;
    }
...
void c_render(OrbitalEyesSandbox* env) {
    if (!env->client) {
...
// ─── Close ───
void c_close(OrbitalEyesSandbox* env) {
    if (env->client) {
        CloseWindow();
        free(env->client);
        env->client = NULL;
